...
import {
  createProcessingJob,
  findActiveProcessingJobsCountByOperationId,
  findLatestProcessingJobByOperationId,
...
import type { ProcessingJob, ProcessingStatus } from '@prisma/client';

const toSafeProcessingJob = (job: ProcessingJob): SafeProcessingJob => ({
  id: job.id,
  ...
  stages: job.stages ? (job.stages as Record<string, any>) : null,
 ...
 */
export const startProcessing = async (userId: string, operationId: string): Promise<SafeProcessingJob> => {
 ...
 */
export const getOperationProcessingStatus = async (userId: string, operationId: string): Promise<SafeProcessingJob> => {
  await verifyOperationOwnership(userId, operationId);
 ...
 */
export const executeProcessingJob = async (userId: string, jobId: string) => {
  const job = await findProcessingJobById(jobId);
  if (!job) throw new ApiError(404, 'Processing job not found.');
