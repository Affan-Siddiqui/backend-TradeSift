...
export const exportExtractionToExcel = async (userId, extractionId) => {
    ...
    const exportData = (extraction.editedFields || extraction.originalFields);
...
export const exportOperationToExcel = async (userId, operationId) => {
    ...
    if (approvedExtractions.length === 1) {
        ...
        if (ext) {
            exportData = (ext.editedFields || ext.originalFields);
        }
    }
    else {
        approvedExtractions.forEach((ext, i) => {
            const data = (ext.editedFields || ext.originalFields);
