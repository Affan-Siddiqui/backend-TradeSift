...
import {
  createOperation,
  findOperationById,
  findOperationsByUserId,
  countOperationsByUserId,
  updateOperationById,
  deleteOperationById,
...
const toSafeOperation = (op: Operation): SafeOperation => ({
  id: op.id,
  userId: op.userId,
 ...
 */
const findOwnedOperation = async (userId: string, operationId: string): Promise<Operation> => {
...
export const updateExistingOperation = async (
  userId: string,
  operationId: string,
  ...
  if (input.status !== undefined) {
    ...
    if (!allowed || !allowed.includes(input.status)) {
      throw new ApiError(
        400,
        `Cannot transition from ${operation.status} to ${input.status}.`
      );
    }
  }

  const updateData: Record<string, unknown> = {};
  if (input.referenceNo !== undefined) updateData.referenceNo = input.referenceNo;
...
export const deleteExistingOperation = async (
  userId: string,
  operationId: string
): Promise<void> => {
