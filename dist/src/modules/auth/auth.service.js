...
import { OTP_EXPIRY_SECONDS, OTP_RESEND_INTERVAL_SECONDS, OTP_MAX_VERIFY_ATTEMPTS, MAX_OTP_GENERATIONS_BEFORE_COOLDOWN, COOLDOWN_LADDER_SECONDS, COOLDOWN_MAX_STAGE, RESET_VERIFIED_TTL_SECONDS, } from './auth.constants.js';
import { setPendingRegistration, getPendingRegistration, deletePendingRegistration, findUserByEmail, createUser, findCooldownRecord, upsertCooldownRecord, deletePendingLogin, getPendingLogin, setPendingLogin, findUserById, updateUserPassword, deletePendingPasswordReset, getPendingPasswordReset, setPendingPasswordReset, } from './auth.repository.js';
import { createSession, deleteSessionByRefreshTokenHash, deleteAllSessionsForUser, updateSessionRefreshToken, findSessionByRefreshTokenHash, } from "../sessions/session.repository.js";
import { countTrustedDevicesForUser, createTrustedDevice, deleteLeastRecentlyUsedTrustedDevice, updateTrustedDeviceLastUsed, deleteAllTrustedDevicesForUser, findTrustedDeviceByHash, } from "../trusted-devices/trustedDevice.repository.js";
...
// ---------- Helpers ----------
const checkCooldown = async (email, type) => {
...
const recordOtpGeneration = async (email, type) => {
    const record = await findCooldownRecord(email, type);
...
const sendOtpEmail = async (email, otp) => {
    await sendMail(email, 'Your TradeSift verification code', `<p>Your verification code is <b>${otp}</b>. It expires in 5 minutes.</p>`);
};
// ---------- Register ----------
...
// ---------- Login ----------
export const loginUser = async (input, trustedDeviceCookie) => {
...
export const logoutUser = async (refreshTokenCookie) => {
    if (!refreshTokenCookie)
        return; // nothing to revoke, treat as already logged out
    const hash = hashToken(refreshTokenCookie);
    await deleteSessionByRefreshTokenHash(hash);
};
// ---------- Change Password ----------
export const changeUserPassword = async (userId, input) => {
    const user = await findUserById(userId);
    if (!user)
        throw new ApiError(404, 'User not found.');
    if (user.password) {
        if (!input.currentPassword)
            throw new ApiError(401, 'current password needed');
...
// ---------- Forgot Password ----------
export const forgotPasswordRequest = async (input) => {
    const user = await findUserByEmail(input.email);
    if (!user)
        return; // silent no-op, generic response regardless
...
export const resendForgotPasswordOtp = async (input) => {
...
export const verifyForgotPasswordOtp = async (input) => {
    const pending = await getPendingPasswordReset(input.email);
    if (!pending)
        throw new ApiError(400, 'Reset session expired. Please start again.');
    if (new Date(pending.otpExpiresAt) < new Date()) {
